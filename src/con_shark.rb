  ...
  if people_json
    people_json.each do |query_user, results|
      results.each do |connection|
        connection_object = OpenStruct.new(connection)
        #todo gen distance text doesn't work anymore...fix
        connection_object.distance_text = generate_distance_text(connection_object.distance)

        ConSharkHelper.add_connection_to_user(full_user_list, connection_object, query_user)
      end
    end
  end
